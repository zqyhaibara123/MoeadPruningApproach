function mop = testmop( testname, dimension,domain)
...
switch lower(testname)
    ...
    case 'nnprun'
        mop=nnprun(mop,dimension,domain);
    case 'nnprun3objs'
        mop=nnprun3objs(mop,dimension,domain);
    otherwise 
...
%NNpruning evaluating
function p=nnprun(p,dim,domain)
 p.name='NNprun';
 p.pd=dim;
 p.od=2;%num of objective
 p.domain=domain;
 p.func=@evaluate;
...
function p=nnprun3objs(p,dim,domain)
 p.name='NNprun3objs';
 p.pd=dim;
 p.od=3;%num of objective
 p.domain=domain;
 p.func=@evaluate;
